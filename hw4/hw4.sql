...
-- Изучим диаграмму связей в бд hw4.

-- 1.Подсчитать общее количество лайков, которые получили пользователи младше 12 лет.

/* Для нахожденния пользователей нужного возраста воспользуемся функцией timestampdiff
 https://sql-academy.org/ru/handbook/TIMESTAMPDIFF 
 */

select  count(*) as 'Всего лайков у пользователей младше 12 лет' from likes
where user_id in (
...
-- Так же мы можем посчитать общее количество лайков для каждого из пола.

select case (gender)
	when 'm' then 'Мужчины'
	when 'f' then 'Женщины'
    end as 'Пол пользователей' , count(*) as 'Всего лайков' 
from profiles 
join likes 
where likes.user_id = profiles.user_id
group by gender;
...
select concat(firstname, ' ', lastname) as 'Пользователи которые не отправляли сообщений' from users
where not exists (
...
-- Зададим переменную в которой будем указывать id интересующего нас пользователя.
...
select  from_user_id as 'id отправителя', 
    ...
    from users 
    where id = messages.from_user_id) as 'Кто отправил', count(*) as `Отправлено сообщений`
from messages 
