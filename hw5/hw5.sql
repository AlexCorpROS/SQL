-- Создадим базу данных hw5 с которой будем работать, используя данные из lesson_4.
...
create or replace view younger_20 as 
...
select dense_rank() over
	(order by count(from_user_id) desc) as 'Место в рейтинге',
	count(from_user_id) as 'Отправленно сообщений',
...
select id, body, created_at,
    timediff(created_at, lag(created_at) over()) as 'Разница даты отправления'
from messages
order by created_at;

-- Удалим не используемое представление younger_20

drop view younger_20;